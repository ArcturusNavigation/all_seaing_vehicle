...
class EStoppedNode(Node):
    ...
    def __init__(self, name="default_e_stopped_node"):
        ...
        self.subscription = self.create_subscription(
            Header, "heartbeat", self.listener_callback, 10
        )
        timer_period = 1
    ...
    def timer_callback(self):
        if (
            self.get_clock().now().to_msg().sec - self.last_received_time
            > self.timeout_duration
        ):
            self.get_logger().info(
                f"Node is e_stopped: has not received in the last {self.timeout_duration} seconds since time: {self.last_received_time}"
            )
