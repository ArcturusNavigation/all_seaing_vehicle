...
class Controller(Node):
    ...
    def update_odometry(self, msg):
        ...
        self.local_pid_vx.update_feedback_value(vx_boat_space)
        self.local_pid_vy.update_feedback_value(vy_boat_space)

        self.last_data_timestamp = self.get_time() # keep a timestamp for the IMU update
    ...
    def update_control_input(self, msg):
        ...
        """
        selected_x_pid = ( 
            self.pid_vx if msg.world_space else self.local_pid_vx
        ) if msg.use_x_velocity else self.pid_x

        selected_y_pid = ( 
            self.pid_vy if msg.world_space else self.local_pid_vy
        ) if msg.use_y_velocity else self.pid_y

        selected_angular_pid = self.pid_omega if msg.use_angular_velocity else self.pid_theta

        if selected_x_pid != self.chosen_x_pid:
            self.chosen_x_pid = selected_x_pid
            self.chosen_x_pid.reset()
        if selected_y_pid != self.chosen_y_pid:
            self.chosen_y_pid = selected_y_pid
            self.chosen_y_pid.reset()
        if selected_angular_pid != self.chosen_angular_pid:
            self.chosen_angular_pid = selected_angular_pid
            self.chosen_angular_pid.reset()
