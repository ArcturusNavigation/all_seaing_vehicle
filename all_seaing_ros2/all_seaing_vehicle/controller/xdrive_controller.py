...
class Controller(Node):
    ...
    def __init__(self):
        ...
        self.i.angular = (
            0.0  # angular parameter - theta if use_heading, otherwise omega
        )
        ...
        self.angular_factor = (
            0.32 if in_sim else 1
        )  # units (kgm^2/s) arbitrary conversion between angular velocity and thrust, determined experimentally
        ...
        self.actual_theta = (
            0  # most recent IMU value for theta (converted from quaternion)
        )

        self.accumulated_error = (
            0  # generic accumulated error value (omega or theta) used for I control
        )
        self.last_error = (
            None  # most recent error value (omega or theta) used for D control
        )
        ...
        self.create_subscription(Imu, imu_topic, self.update_heading, 10)
    ...
    def update_thrust(self):
        ...
        angular_input = (
            0 if self.i.use_heading else self.i.angular
        )  # this represents what we will pass in as angular velocity
        ...
        results = self.get_thrust_values(
            self.i.vx,
            self.i.vy,
            self.restrict_input(angular_input, self.max_angular_velocity),
        )
