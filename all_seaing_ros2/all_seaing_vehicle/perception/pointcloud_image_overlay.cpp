...
void PclImageOverlay::pc_image_fusion_cb(const sensor_msgs::msg::Image::ConstSharedPtr &in_img_msg,
                                         const sensor_msgs::msg::PointCloud2::ConstSharedPtr &in_cloud_msg)
{
    ...
    for (pcl::PointXYZI &point_tf : in_cloud_tf_ptr->points)
    {
        ...
        if ((xy_rect.x >= 0) && (xy_rect.x < m_cam_model.cameraInfo().width) &&
            (xy_rect.y >= 0) && (xy_rect.y < m_cam_model.cameraInfo().height) && (point_tf.x >= 0))
        {
...
PclImageOverlay::PclImageOverlay() : Node("pointcloud_image_overlay")
{
    ...
    m_image_sub.subscribe(this, "/img_src", rmw_qos_profile_system_default);
    m_cloud_sub.subscribe(this, "/cloud_src", rmw_qos_profile_system_default);

    // Send pc msg and img msg to PCImageFusionCb
    m_pc_cam_sync = std::make_shared<PointCloudCamSync>(PointCloudCamPolicy(10), m_image_sub, m_cloud_sub);
    m_pc_cam_sync->registerCallback(std::bind(&PclImageOverlay::pc_image_fusion_cb, this, std::placeholders::_1, std::placeholders::_2));

    // Publishers
    m_image_pub = this->create_publisher<sensor_msgs::msg::Image>("/pc_image_fusion", 1);
}
