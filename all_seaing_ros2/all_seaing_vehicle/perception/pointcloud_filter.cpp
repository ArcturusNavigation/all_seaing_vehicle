...
public:
    PointCloudFilter() : Node("point_cloud_filter")
    {
        ...
        m_subscription = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            ...
            "/wamv/sensors/lidars/filtered/points", 10, std::bind(&PointCloudFilter::pc_callback, this, std::placeholders::_1));
...
private:
    ...
    void pc_callback(const sensor_msgs::msg::PointCloud2 &in_cloud_msg)
    {
        ...
        for (pcl::PointXYZI &pt : in_cloud_ptr->points)
        {
        ...
        if (m_leaf_size != 0) downsampleCloud(filtered_cloud_ptr, downsampled_cloud_ptr);
