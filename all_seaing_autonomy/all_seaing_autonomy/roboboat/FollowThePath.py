    ...
    def angle_with(self, other):
        return acos(
            max(
                -1,
                min(
                    1, self.dot_product(other) / (self.magnitude() * other.magnitude())
                ),
            )
        )
...
def get_ordering_item(buoy_list, ordering, index, is_red, stepback):
    return (
        Vector(diff_color_midpoint_dist / 2 * (1 if is_red else -1), 1 - stepback)
        if index < 0
        else buoy_list[ordering[index]].position
    )


def get_reference(buoy_list, ordering, is_red, stepback=1):
    return get_ordering_item(
        buoy_list, ordering, len(ordering) - stepback, is_red, stepback
    )


def get_normal(buoy_list, red_index, green_index):
    return Vector(
        buoy_list[green_index].position.y - buoy_list[red_index].position.y,
        buoy_list[red_index].position.x - buoy_list[green_index].position.x,
    )


def get_weight_from_pair(
    buoy_list, green_index, red_index, green_ordering, red_ordering, red_ref, green_ref
):
    between_buoys_diff = buoy_list[red_index].position - buoy_list[green_index].position
    diff_color_dist_unweighted = abs(
        between_buoys_diff.magnitude() - diff_color_midpoint_dist
    )
    ...
    vector_to_point = (
        buoy_list[red_index].position
        + buoy_list[green_index].position
        - green_ref
        - red_ref
    ) / 2
    ...
    previous_vector = (
        green_ref + red_ref - green_secondary_ref - red_secondary_ref
    ) / 2
    ...
    if consider_angle:
        outgoing_angle_unweighted = get_normal(
            buoy_list, red_index, green_index
        ).angle_with(vector_to_point)

    if outgoing_angle_unweighted > outgoing_angle_max:
        return None

    return (
        diff_color_dist_unweighted * diff_color_dist_weight
...
def get_individual_weight(buoy_list, index, ordering, ref):
    ...
    distance_feet = abs(
        diff.magnitude() - (same_color_midpoint_dist if ordering else 0)
    )
    if not ordering:
...
def choose_next_pair(buoy_list, green_set, red_set, green_ordering, red_ordering):
    ...
    for index in green_set:  # green loop
        individual_weight = get_individual_weight(
            buoy_list, index, green_ordering, green_ref
        )
        if individual_weight is not None:
            for r_index in red_set:
                if r_index not in red_cache:
                    red_cache[r_index] = get_individual_weight(
                        buoy_list, index, red_ordering, red_ref
                    )
                r_individual_weight = red_cache[r_index]
                if r_individual_weight is not None:
                    multicolor_weight = get_weight_from_pair(
                        buoy_list,
                        index,
                        r_index,
                        green_ordering,
                    ...
                    if multicolor_weight is not None:
                        total_weight = (
                            individual_weight + r_individual_weight + multicolor_weight
                        )
...
def generate_path(_buoy_list):
    ...
    for cluster in _buoy_list:
        ...
        buoy_list.append(
            Buoy(Vector(cluster.avg_point.x, cluster.avg_point.y), cluster.label == 2)
        )
    print([buoy.__str__() for buoy in buoy_list])
    while True:
        pair = choose_next_pair(
            buoy_list, green_buoys, red_buoys, green_ordering, red_ordering
        )
        ...
        print(
            "chose green", buoy_list[pair.green_index], "red", buoy_list[pair.red_index]
        )
        waypoints.append(
            (buoy_list[pair.green_index].position + buoy_list[pair.red_index].position)
            / 2
        )
    return waypoints
