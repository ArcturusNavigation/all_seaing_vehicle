...
class ReplayRecordings(Node):
            ...
            if not self.enter_held:
                ...
                if self.publish_from_pickles(self.counter):
                    self.get_logger().info(
                        f"proceeding to recording {self.counter}, press enter for next"
                    )
                    self.counter += 1
                else:
                    if self.counter == 1:
                    ...
                    self.get_logger().info(
                        "finished viewing all recordings, press enter to cycle back to 1"
                    )
                    self.counter = 1
        else:
            self.enter_held = False
    ...
    def publish_from_pickles(self, num):
        ...
        if not os.path.exists(os.path.join(self.path_value, str(num))):
            return False
        with open(
            os.path.join(self.path_value, str(num), "path.pkl"), "rb"
        ) as path_file:
            path_msg = pickle.load(path_file)
        with open(
            os.path.join(self.path_value, str(num), "markers.pkl"), "rb"
        ) as marker_file:
            markers_msg = pickle.load(marker_file)  # include "rcl"
