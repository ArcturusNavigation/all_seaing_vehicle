...
class PIDController:
    ...
    def is_done(self, threshold):
        return abs(self.setpoint - self.feedback) <= threshold
...
class CircularPID(PIDController):
    def is_done(self, threshold):
        feedback_mod = self.feedback % (2*math.pi)
    ...
    def update(self, feedback, dt):
        if dt == 0:
            return
        self.feedback = feedback
        error = (self.setpoint - self.feedback) % (2 * math.pi)
        if error > math.pi:
            error -= 2 * math.pi
        derivative = (error - self.prev_error) / dt
        self.integral += error * dt
