...
class XDriveController(Node):
    ...
    def __init__(self):
        ...
        pid_omega = PID(
            (2, 0.5, 0) if self.in_sim else (0.0, 0.00, 0), self.get_omega_feedback
        )  # 0.8, 0.04, 0
        pid_theta = CircularPID(
            (1, 0, 0) if self.in_sim else (0.0, 0, 0), self.get_theta_feedback
        )

        pid_x = PID(
            (0.1, 0, 0) if self.in_sim else (0.0, 0, 0),
            self.get_x_position_feedback,
            None,
        )
        pid_y = PID(
            (0.1, 0, 0) if self.in_sim else (0.0, 0, 0),
            self.get_y_position_feedback,
            None,
        )
        pid_vx = PID(
        ...
        if BLIND_LINEAR:
            imu_topic_name = (
                "/wamv/sensors/imu/imu/data" if self.in_sim else "/mavros/imu/data"
            )
            if self.in_sim:
    ...
    def get_x_velocity_world_feedback(self, msg):
        return msg.twist.twist.linear.x * math.cos(
            self.theta
        ) - msg.twist.twist.linear.y * math.sin(self.theta)

    def get_y_velocity_world_feedback(self, msg):
        return msg.twist.twist.linear.y * math.cos(
            self.theta
        ) + msg.twist.twist.linear.x * math.sin(self.theta)
    ...
    def update_odometry(self, msg):
        ...
        # self.get_logger().info("updating x feedback")
        self.x_control_handlers[self.chosen_linear_control_mode].update_feedback_value(
            msg
        )
        self.y_control_handlers[self.chosen_linear_control_mode].update_feedback_value(
            msg
        )

    def update_theta(self, msg, orientation_msg):
        self.theta = R.from_quat(
        ...
        self.angular_control_handlers[
            self.chosen_angular_control_mode
        ].update_feedback_value(msg)
    ...
    def convert_to_pwm_and_send(self, thrust_values):
        ...
        for name in self.all_thruster_names:
            ...
            float_msg.data = self.py_type(
                self.restrict_input(
                    thrust_values[name],
                    self.max_input,  # make sure the input isn't way out of bounds
                )
                * self.thrust_factor
                + self.midpoint
    ...
    def update_thrust(self):
        ...
        if self.last_update_timestamp is not None:
            ...
            if (
                self.last_data_timestamp is not None
                and self.time_diff(current_time, self.last_data_timestamp)
                <= self.required_data_recentness
            ):
                x_output = self.x_control_handlers[
                    self.chosen_linear_control_mode
                ].determine_output(self.x_input, dt)
                y_output = self.y_control_handlers[
                    self.chosen_linear_control_mode
                ].determine_output(self.y_input, dt)
                angular_output = self.angular_control_handlers[
                    self.chosen_angular_control_mode
                ].determine_output(self.angular_input, dt)

            # self.get_logger().info(f"raw pid output: ({x_output}, {y_output})")

            if (
                self.chosen_linear_control_mode == ControlMessage.WORLD_VELOCITY
                or self.chosen_linear_control_mode == ControlMessage.WORLD_POSITION
            ):
                x_boat_space = x_output * math.cos(self.theta) + y_output * math.sin(
                    self.theta
                )
                y_boat_space = y_output * math.cos(self.theta) - x_output * math.sin(
                    self.theta
                )
            else:
            ...
            # self.get_logger().info(f"boat: ({x_boat_space}, {y_boat_space})")
            results = self.get_thrust_values(x_boat_space, y_boat_space, angular_output)
            # self.get_logger().info(f"thrust values: {results}")
