...
class Yolov11Node_shapes_buoys(Node):

    def __init__(self):
        super().__init__("yolov11_node_shapes_buoys")

        perception_prefix = get_package_share_directory("all_seaing_perception")
        bringup_prefix = get_package_share_directory("all_seaing_bringup")

        # Declare/get ROS parameters
        # Have both best.pt and last.pt
        # Want to test both?
        model_name_buoy = self.declare_parameter(
            "model", "best").get_parameter_value().string_value
        model_name_shape = self.declare_parameter(
            "model", "roboboat_shape_2025").get_parameter_value().string_value
        ...
        engine_path_buoy = os.path.join(perception_prefix, "models", model_name_buoy + ".engine")
        pt_path_buoy = os.path.join(perception_prefix, "models", model_name_buoy + ".pt")
        engine_path_shape = os.path.join(perception_prefix, "models", model_name_shape + ".engine")
        pt_path_shape = os.path.join(perception_prefix, "models", model_name_shape + ".pt")

        # Initialize YOLO model
        # check if we are using the jetson and if the engine path exists.
        # Disable tensorRT engine portion??
        self.cv_bridge = CvBridge()
        # Loads buoy model
        if os.path.exists("/etc/nv_tegra_release") and os.path.isfile(engine_path_buoy):
            self.get_logger().info(f"Loading buoy model from tensorRT engine: {engine_path_buoy}")
            self.model_buoy = YOLO(engine_path_buoy)
        elif os.path.isfile(pt_path_buoy):
            self.get_logger().info(f"Loading buoy model from pt model: {pt_path_buoy}")
            self.model_buoy = YOLO(pt_path_buoy)
        else:
            self.get_logger().error(f"Both buoy model paths do not exist :( TensorRT: {engine_path_buoy} and pt: {pt_path_buoy}")

        # Loads shape model
        if os.path.exists("/etc/nv_tegra_release") and os.path.isfile(engine_path_shape):
            self.get_logger().info(f"Loading shape model from tensorRT engine: {engine_path_shape}")
            self.model_shape = YOLO(engine_path_shape)
        elif os.path.isfile(pt_path_shape):
            self.get_logger().info(f"Loading shape model from pt model: {pt_path_shape}")
            self.model_shape = YOLO(pt_path_shape)
        else:
            self.get_logger().error(f"Both shape model paths do not exist :( TensorRT: {engine_path_shape} and pt: {pt_path_shape}")
        ...
        # Put image from subscription through HSV filtering to help accuracy??
        # Still have the one from HSV filtering before
    ...
    def image_cb(self, msg: Image) -> None:
        ...
        # Append Buoy Results
        pred_results_buoy = self.model_buoy.predict(
            source=cv_image,
            verbose=False,
            stream=False,
            conf=self.conf,
        ...
        for box_data in results_buoy.boxes:
            ...
            if results_buoy.boxes:
                ...
                label_name = self.model_buoy.names[int(box_data.cls)]

                labeled_bounding_box_msgs.boxes.append(box_msg)

                class_name = f"{label_name}_{str(int(box_data.cls))}"
                # Only adds the objects detected that are buoys
                if "buoy" in class_name:
        ...
        # Assumed would use same code for annotating as the buoy model
        # Might have problem with two bounding boxes for one buoy/shape?
        pred_results_shape = self.model_shape.predict(
            source=cv_image,
            verbose=False,
            stream=False,
            conf=self.conf,
        ...
        results_shape: Results = pred_results_shape[0].cpu()

        for box_data in results_shape.boxes:
            ...
            if results_shape.boxes:
                ...
                box_msg.min_y = int(center_y - height / 2)
                box_msg.max_y = int(center_y + height / 2)
                # Uses names from only one model name set
                label_name = self.model_buoy.names[int(box_data.cls)]

                labeled_bounding_box_msgs.boxes.append(box_msg)

                # Adds all bouding boxes detected by model
                class_name = f"{label_name}_{str(int(box_data.cls))}"
                class_name_list = class_name.split("_")
                color_name = class_name_list[0] # assumes labels follow "color_object" format
                color = ()
        ...
        # Publish detections for both buoy and shape on one image
