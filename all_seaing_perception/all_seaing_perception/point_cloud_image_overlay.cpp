...
void PclImageOverlay::pc_image_fusion_cb(
    ...
    if (!m_pc_cam_tf_ok)
        m_pc_cam_tf =
            get_tf(in_img_msg->header.frame_id, in_cloud_msg->header.frame_id);
    ...
    tf2::doTransform<sensor_msgs::msg::PointCloud2>(*in_cloud_msg, in_cloud_tf,
                                                    m_pc_cam_tf);
    pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_tf_ptr(
    ...
    for (pcl::PointXYZI &point_tf : in_cloud_tf_ptr->points) {
        ...
        if ((xy_rect.x >= 0) && (xy_rect.x < m_cam_model.cameraInfo().width) &&
            ...
            (point_tf.x >= 0)) {
            cv::circle(cv_ptr->image, cv::Point(xy_rect.x, xy_rect.y), 2,
                       cv::Scalar(255, 0, 0), 4);
        }
    }
...
PclImageOverlay::get_tf(const std::string &in_target_frame,
