...
ObstacleBboxVisualizer::ObstacleBboxVisualizer() : Node("obstacle_bbox_visualizer") {
    ...
    if (yamlFile.is_open()) {
        ...
        for (YAML::const_iterator it = config_yaml.begin(); it != config_yaml.end(); ++it) {
            // RCLCPP_INFO(this->get_logger(), "label %d and color %s", it->second.as<int>(),
        ...
        RCLCPP_ERROR(this->get_logger(), "Failed to open YAML file: %s",
                     color_label_mappings_file.c_str());
    }
...
void ObstacleBboxVisualizer::image_obstacle_cb(
    ...
    for (const auto &obstacle : in_map_msg->obstacles) {
        ...
        cv::putText(cv_ptr->image, label_text, label_position, cv::FONT_HERSHEY_SIMPLEX, 0.5, color,
                    1);
...
void ObstacleBboxVisualizer::intrinsics_cb(
    const sensor_msgs::msg::CameraInfo::ConstSharedPtr &info_msg) {
    m_cam_model.fromCameraInfo(info_msg);
}

geometry_msgs::msg::TransformStamped
ObstacleBboxVisualizer::get_tf(const std::string &in_target_frame,
