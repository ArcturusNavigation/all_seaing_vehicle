...
public:
    NavStateReporter() : Node("nav_state_reporter") {
        ...
        m_timer = this->create_wall_timer(
            200ms, std::bind(&NavStateReporter::timer_callback, this));
        ...
        m_gateway_sub =
            this->create_subscription<protobuf_client_interfaces::msg::Gateway>(
                "/gateway_msg", 10,
                std::bind(&NavStateReporter::gateway_callback, this,
                          std::placeholders::_1));

        // Publishers
        m_state_pub = this->create_publisher<all_seaing_interfaces::msg::ASVState>(
            "asv_state", 10);
        m_gateway_pub =
            this->create_publisher<protobuf_client_interfaces::msg::Gateway>(
                "/send_to_gateway", 10);
    }
...
private:
    ...
    all_seaing_interfaces::msg::ASVState m_state =
        all_seaing_interfaces::msg::ASVState();
    ...
    void imu_callback(const sensor_msgs::msg::Imu &msg) {
        ...
        m_state.nav_heading =
            -y * 180 / M_PI +
            90; // Offset and negative sign due to NED/ENU IMU difference
    }
    ...
    void gateway_callback(const protobuf_client_interfaces::msg::Gateway &msg) {
        if (msg.gateway_key == "DESIRED_RUDDER")
        ...
        if (msg.gateway_key == "NAV_Y")
            m_state.nav_y = msg.gateway_double;
    }
    ...
    void timer_callback() {
        ...
        gateway_msg.gateway_string =
            "NAV_LAT=" + std::to_string(m_state.nav_lat) +
            ", NAV_LON=" + std::to_string(m_state.nav_long) +
            ", NAV_HEADING=" + std::to_string(m_state.nav_heading) +
            ", NAV_SPEED=" + std::to_string(m_state.nav_speed);
